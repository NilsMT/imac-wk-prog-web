function loadComments(eventId) {
  ...
  fetch(`/api/v1/comments/${eventId}`)
    ...
    .then((comments) => {
      ...
      const fullName = (c) => `${c.firstname} ${c.name}` || c.pseudo || "Utilisateur inconnu";
...
function sendComment(eventId) {
  ...
  fetch(`/api/v1/comments/${eventId}`, {
    ...
    .catch((error) => {
      showError(`comments-container-${eventId}`, "Une erreur est survenue. Veuillez réessayer.");
    });
}
  ...
  fetch(`/api/v1/participations/${eventId}`, {
    ...
    .then((data) => {
      if (data.message.includes("success")) {
        ...
        document.getElementById(`participationToggle-${eventId}`).checked = !isParticipating;
        showError(`eventCard-${eventId}`, data.message);
      }
    })
    .catch(() => {
      document.getElementById(`participationToggle-${eventId}`).checked = !isParticipating;
      showError(`eventCard-${eventId}`, "Une erreur est survenue. Veuillez réessayer.");
    });
}
              ...
              <div class="card-body p-4">
                ...
                <div class="d-flex justify-content-between mb-3 text-muted align-items-center">
                  <div class="d-flex align-items-center">
                    ...
                    ${event.end_date.substring(0, 10) === event.start_date.substring(0, 10)
                ...
                ${event.attributes && event.attributes.length > 0
                  ? `<div class="mb-3">${event.attributes.map(attr => `
                      <div class="d-flex align-items-center">
