function loadComments(eventId) {
  ...
  fetch(`/api/v1/comments/${eventId}`)
    ...
    .then((comments) => {
      ...
      const fullName = (comment) =>
        `${comment.firstname} ${comment.name}` ||
        `${comment.pseudo}` ||
        "Utilisateur inconnu";
    ...
    .catch((error) => {
      ...
      commentsContainer.innerHTML = `<div class="alert alert-danger">Impossible de charger les messages.</div>`;
    });
}
        ...
        <div class="row g-4">
          ...
          <div class="col-md-7">
            <div class="card shadow-lg h-100" id="eventCard-${eventId}">
              ${
                event.image
                  ? `<img src="${event.image}" class="card-img-top" alt="Image de l'événement" style="height: 180px; object-fit: cover" />`
                  : `<img src="${window.location.origin}/static/uploads/default.jpg" class="card-img-top" alt="Image de l'événement" style="height: 180px; object-fit: cover" />`
              }
              <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center mt-3">
                  <label class="switch m-0">
                    <input
                      type="checkbox"
                      id="participationToggle-${eventId}"
                      ${isParticipating ? "checked" : ""}
                      onchange="toggleParticipation(${eventId}, this.checked)"
                    />
                    <span class="slider round"></span>
                  </label>
                  <span class="ms-3" id="participationLabel-${eventId}">
              ...
              <div class="card-footer p-3">
                <div class="input-group input-group-outline">
                  <input
                    ...
                    onkeypress="if(event.key === 'Enter') sendComment(${eventId})"
                  />
                  <button
                    class="btn btn-info m-0"
                    onclick="sendComment(${eventId})"
                  >
    ...
    .catch((error) => {
      ...
      modalBody.innerHTML = `
        <div class="alert alert-danger">
          Impossible de charger les détails de l'événement ou votre statut de participation.
        </div>
      `;
    });
}
...
function toggleParticipation(eventId, isParticipating) {
  const method = isParticipating ? "POST" : "DELETE";
  ...
  fetch(`/api/v1/participations/${eventId}`, {
    ...
    .catch((error) => {
      console.error("Erreur:", error);
      const toggle = document.getElementById(`participationToggle-${eventId}`);
      toggle.checked = !isParticipating;
      alert("Une erreur est survenue. Veuillez réessayer.");
    });
}
